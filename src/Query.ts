...
export function Not(c: Component) {
  return () => [c, "not"];
}
export function Or(c: Component) {
  return () => [c, "or"];
}
export function Changed(c: Component) {
  return () => [c, "changed"];
}
 ...
 */
export const enterQuery = (query: QueryFunction) => (world: World) => {
  if (!world[$queryMap].has(query)) registerQuery(world, query);
  const q = world[$queryMap].get(query);
  // queryCommitRemovals(q)
  const entered = q?.entered.dense.slice();
  if (q) q.entered = SparseSet();
  return entered;
};
 ...
 */
export const exitQuery = (query: QueryFunction) => (world: World) => {
  if (!world[$queryMap].has(query)) registerQuery(world, query);
  const q = world[$queryMap].get(query);
  // queryCommitRemovals(q)
  const exited = q?.exited.dense.slice();
  if (q) q.exited = SparseSet();
  return exited;
};
 ...
 *
 */
export const registerQuery = <TWorld extends World>(
  world: TWorld,
  query: QueryFunction<TWorld>
) => {
    ...
    .map(
      (c) =>
        (Object.getOwnPropertySymbols(c).includes($storeFlattened)
          ? c[$storeFlattened]
          : [c]) as unknown as (TypeStore | ArrayStore)[]
    )
    .reduce((a, v) => a.concat(v), []);

  const shadows: unknown[] = [];

  const q: Query = Object.assign(sparseSet, {
    archetypes,
    changed,
    components,
    notComponents,
  ...
  world[$queryMap].set(query as QueryFunction, q);
  world[$queries].add(q);
...
const generateShadow = (q: Query, pid: number) => {
  const $ = Symbol();
  const prop = q.flatProps[pid] as any;
  createShadow(prop, $);
  q.shadows[pid] = prop[$];
  return prop[$];
};

const diff = (q: Query, clearDiff: boolean) => {
  ...
  for (let i = 0; i < q.dense.length; i++) {
    ...
    for (let pid = 0; pid < flatProps.length; pid++) {
      const prop = flatProps[pid] as any;
...
const flatten = <T>(a: T[], v: T) => a.concat(v);

const aggregateComponentsFor = <T>(mod: () => Object) => (x: T[]) =>
  x.filter((f: T) => f.name === mod().constructor.name).reduce(flatten);
...
export const queryCheckComponent = (q: Query, c: RegisteredComponent) => {
 ...
 */
export const resetChangedQuery = (world: World, query: QueryFunction) => {
  const q = world[$queryMap].get(query);
  if (q) q.changed = [];
};
 ...
 */
export const removeQuery = (world: World, query: QueryFunction) => {
  const q = world[$queryMap].get(query);
