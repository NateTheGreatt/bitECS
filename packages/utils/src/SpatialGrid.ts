import { type IUint32SparseSet, createUint32SparseSet, sparseSetAdd, sparseSetRemove } from "./Uint32SparseSet"
...
export function spatialGridRefresh(grid: ISpatialGrid, x: number, y: number, cellIndex: number, id: number): number {
    spatialGridRemove(grid, cellIndex, id)
    return spatialGridAdd(grid, x, y, id)
}

export function spatialGridBroadphaseRadius(grid: ISpatialGrid, cellIndex: number, radius = 3): number[] {
...
export function spatialGridBroadphasePosition(grid: ISpatialGrid, x: number, y: number, r: number): number[] {
    const cx: number = spatialGridToCell(grid, x)
    const cy: number = spatialGridToCell(grid, y)
    const cellIndex: number = spatialGridIndexOf(grid, cx, cy)
...
export function spatialGridBroadphaseView(grid: ISpatialGrid, x: number, y: number, cellsWide: number, cellsHigh: number): number[] {
    const cx: number = spatialGridToCell(grid, x)
    const cy: number = spatialGridToCell(grid, y)
...
export function spatialGridBroadphaseCell(grid: ISpatialGrid, x: number, y: number, cellsWide: number, cellsHigh: number): Cell[] {
    const cx: number = spatialGridToCell(grid, x)
    const cy: number = spatialGridToCell(grid, y)
    const startGridX: number = Math.ceil(cx - cellsWide / 2)
    const startGridY: number = Math.ceil(cy - cellsHigh / 2)
