...
import { TYPES, TYPES_ENUM, TYPES_NAMES, UNSIGNED_MAX } from '../constants/Constants.js';
...
const createTypeStore = (type: keyof typeof TYPES, length: number) => {
	...
	const buffer = isSabSupported() ? new SharedArrayBuffer(totalBytes) : new ArrayBuffer(totalBytes);
...
const createArrayStore = (metadata: Metadata, type: keyof typeof TYPES, length: number) => {
	...
	if (!metadata[$storeSubarrays][type]) {
		...
		const totalBytes =
			roundToMultiple4(arrayElementCount * storeSize) * TYPES[type].BYTES_PER_ELEMENT;

		const buffer = isSabSupported()
			? new SharedArrayBuffer(totalBytes)
			: new ArrayBuffer(totalBytes);

		const array = new TYPES[type](buffer) as TODO;
		array[$indexType] = TYPES_NAMES[indexType];
		array[$indexBytes] = TYPES[indexType].BYTES_PER_ELEMENT;
