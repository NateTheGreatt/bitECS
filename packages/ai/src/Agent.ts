...
import { $schema } from '@bitecs/core/src/component/symbols';
...
const ecsAddEntity =
	(
		world: World,
		{ components: componentMap, relations: relationMap, entities: entityMap }: CreateAgentParams
	) =>
	({ name }: { name: string }) => {
...
const ecsGetRelationValues =
	...
	({ relationName, relationSubjectEntityId, propertyNames, entityId }: GetRelationValuesParams) => {
		const obj: any = {};
		const relation = relationMap[relationName];
		const relationComponent = Pair(relation, relationSubjectEntityId);
		for (const propertyName of propertyNames) {
...
const ecsQuery =
	...
	({ componentNames, notComponentNames, relations, notRelations }: QueryParams) => {
		...
		const relationPairs = relations
			? relations.map(({ relationName, relationSubjectEntityId: entityId }) => {
					const eid =
						typeof entityId === 'string' ? entityMap[entityId] : parseInt(entityId);
					return Pair(relationMap[relationName], eid);
			  })
			: [];
		const notRelationPairs = notRelations
			? notRelations.map(({ relationName, relationSubjectEntityId: entityId }) => {
					const eid =
						typeof entityId === 'string' ? entityMap[entityId] : parseInt(entityId);
...
export const createAgent = (params: CreateAgentParams) => {
	...
	const componentDefinitions = Object.entries(componentMap).reduce((defs, [name, component]) => {
