...
var stores = {};
var resize = (typedArray, size) => {
  const newBuffer = new ArrayBuffer(size * typedArray.BYTES_PER_ELEMENT);
  const newTa = new typedArray.constructor(newBuffer);
...
var resetStoreFor = (store, eid) => {
  if (store[$storeFlattened]) {
    store[$storeFlattened].forEach((typedArray) => {
      if (ArrayBuffer.isView(typedArray))
        typedArray[eid] = 0;
      else
        typedArray[eid].fill(0);
    });
  }
};
...
var createStore = (schema, size) => {
  ...
  if (schema instanceof Object && Object.keys(schema).length) {
    const recursiveTransform = (a, key) => {
      if (typeof a[key] === "string") {
        ...
        a[key] = createArrayStore(metadata, type, length);
        a[key][$storeBase] = () => stores[$store];
        metadata[$storeFlattened].push(a[key]);
...
var registerQuery = (world, query) => {
  ...
  const flatProps = components2.filter((c) => !c[$tagStore]).map(
    (c) => Object.getOwnPropertySymbols(c).includes($storeFlattened) ? c[$storeFlattened] : [c]
    ...
    for (let pid = 0; pid < componentProps.length; pid++) {
      ...
      for (let i = 0; i < ents.length; i++) {
        ...
        const newlyAddedComponent = (
          // if we are diffing
          shadow && // and we have already iterated over this component for this entity
          // retrieve cached value
          cache.get(component).get(eid) || // or if entity did not have component last call
          !componentCache.has(component) && // and entity has component this call
...
var defineDeserializer = (target) => {
  ...
  const deserializer = (world, packet, mode = 0) => {
    newEntities.clear();
    if (resized) {
